...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		FlxG.save.bind('weekend_exciting', 'foxgamin_');
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			// music.loadStream(Paths.music('${CoolUtil.titleSongName}'));
			// FlxG.sound.list.add(music);
			// music.play();

			if (FlxG.sound.music == null)
			{
				FlxG.sound.playMusic(Paths.music('${CoolUtil.titleSongName}'), 0);
			}
		}
		...
		orange1 = new FlxSprite().loadGraphic(Paths.image('title/up'));
		orange1.scrollFactor.set(0, 0);
		orange1.updateHitbox();
		orange1.screenCenter();
		orange1.antialiasing = true;
		add(orange1);

		orange2 = new FlxSprite().loadGraphic(Paths.image('title/down'));
		orange2.scrollFactor.set(0, 0);
		orange2.updateHitbox();
		orange2.screenCenter();
		orange2.y -= 720;
		orange2.antialiasing = true;
		add(orange2);

		outline = new FlxSprite().loadGraphic(Paths.image('title/outline'));
	...
	override function update(elapsed:Float)
	{
		...
		{
			orange1.y += 50 * elapsed;
			orange2.y += 50 * elapsed;
